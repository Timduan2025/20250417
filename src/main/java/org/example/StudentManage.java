...
public class StudentManage {
    public static void main(String[] args) {
        ...
        double engAvg = calculateAverage(englishScores);
        double sciAvg = calculateAverage(scienceScores);

        System.out.println("數學平均：" + mathAvg);
        System.out.println("英語平均：" + engAvg);
        System.out.println("科學平均：" + sciAvg);

        // 找出各科最高分的學生
        System.out.println("\n各科最高分：");
        findTopScores(names, mathScores, "數學");
        findTopScores(names, englishScores, "英語");
        findTopScores(names, scienceScores, "科學");
    ...
    // 找出各科最高分的學生
    public static void findTopScores(String[] names, int[] scores, String subject) {
    ...
    // 顯示總分排名
    public static void displayTotalScoreRanking(String[] names, int[] mathScores, int[] englishScores, int[] scienceScores) {
        //計算所有學生的總分
        int[] totalScores = new int[names.length];
        for (int i = 0; i < names.length; i++) {
            totalScores[i] = mathScores[i] + englishScores[i] + scienceScores[i];
        }
        ...
        // 根據總分對索引進行排序
        sortByTotalScores(indices, totalScores);
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            System.out.println((i + 1) + "\t" + names[idx] + "\t" + totalScores[idx] + "\t" + formatScore(average));
        }
    }

    // 根據總分對索引進行排序 (從高到低)
    public static void sortByTotalScores(int[] indices, int[] totalScores) {
        for (int i = 0; i < indices.length; i++) {
